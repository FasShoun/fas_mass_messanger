...
const loginVerify = async (req, res, next) => {
  try {
    ...
    if (passCheck) {
      ...
      if(token){
        res.render("fasMass",{name:userGmailMatch.fullName,hi_name:userGmailMatch.userName,title:`Welcome ${userGmailMatch.fullName}`,image:`${userGmailMatch.upFile.fileName}`});    
      }else{
        res.render("login")
      }
